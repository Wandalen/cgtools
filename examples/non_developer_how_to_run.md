# How to run a rust-wasm example
...
Go to the [GitHub releases page](https://github.com/trunk-rs/trunk/releases/tag/v0.21.14)
...
Go to `examples/minwebgl` and find the `jewelry_3d_site` folder. You need to open a terminal in this directory.
...
As a result, you should see a terminal window with the path pointing to this folder. For example, on my system it looks like:

```text
PS C:\Users\Admin\Documents\repos\cgtools\examples\minwebgl\jewelry_3d_site>
```

Your path will be different, but the `examples\minwebgl\jewelry_3d_site` part should match exactly.
...
When the page is loaded, you should see a ring in the background and some text in the foreground.
You should be able to smoothly scroll down until you see the **“Customize it”** button, and also scroll back up to the first title.

Make sure scrolling works smoothly in both directions and does not lag or get stuck.

Next, click **“Customize it”**. You should see a smooth transition into the configuration menu.
You should be able to rotate and zoom the camera using your mouse. In the center of the scene, you should see a silver ring with a white diamond, without any visual artifacts.

Try changing every available parameter using the three buttons at the bottom.
For example, change the diamond color and the ring color, then switch the ring model.
The new ring model should have default silver and white colors, but if you switch back to the first ring, it should remember the colors you previously set.

If you notice a bug or something doesn’t work as intended, please try to record it using a screen recording tool with the browser debug console open (press `F12` while on the page).
