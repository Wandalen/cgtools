...
mod tests
{
  ...
  fn scene_rhai_parses_and_matches_known_values()
  {
    ...
    assert_eq!( scene.background.top.to_array(), [ 0.0196, 0.0549, 0.0941, 1.0 ] );
    assert_eq!( scene.background.bottom.to_array(), [ 0.0549, 0.1490, 0.2392, 1.0 ] );

    assert_eq!( scene.nebula.color.to_array(), [ 0.0706, 0.2000, 0.2902, 1.0 ] );
    assert_eq!( scene.nebula.opacity, 0.45 );

    assert_eq!( scene.stars.color.to_array(), [ 0.6275, 0.8980, 1.0, 1.0 ] );
    assert_eq!( scene.stars.intensity, 0.6 );

    assert_eq!( scene.grid.color.to_array(), [ 0.3137, 0.5490, 0.7451, 1.0 ] );
    assert_eq!( scene.grid.opacity, 0.18 );

    assert_eq!( scene.sun_corona.inner.to_array(), [ 1.0, 0.8941, 0.4392, 1.0 ] );
    assert_eq!( scene.sun_corona.mid.to_array(), [ 1.0, 0.6824, 0.1020, 1.0 ] );
    assert_eq!( scene.sun_corona.outer.to_array(), [ 1.0, 0.2314, 0.0, 1.0 ] );

    assert_eq!( scene.sun_disc.dark.to_array(), [ 1.0, 0.4157, 0.0, 1.0 ] );
    assert_eq!( scene.sun_disc.mid.to_array(), [ 1.0, 0.8941, 0.4392, 1.0 ] );
    assert_eq!( scene.sun_disc.bright.to_array(), [ 1.0, 1.0, 1.0, 1.0 ] );
    assert_eq!( scene.sun_disc.base_radius, 0.075 );

    assert_eq!( scene.orbit_ring.color.to_array(), [ 0.3922, 0.8235, 1.0, 1.0 ] );
