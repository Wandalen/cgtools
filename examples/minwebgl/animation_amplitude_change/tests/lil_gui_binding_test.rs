...
//!
//! `animation_amplitude_change` is a binary-only example crate (no `[lib]` target), so this test
//! reads both files' own real source text via `include_str!` rather than exercising the
//! wasm-only bindings.
