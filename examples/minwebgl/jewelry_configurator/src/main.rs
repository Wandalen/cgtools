...
async fn run()
{
  ...
  // Renderer setup
  let mut renderer = match JewelryRenderer::new( &canvas, "environment_maps/studio", "environment_maps/studio3/env-gem-4.hdr", true ).await
  {
    Ok( r ) => r,
    Err( err ) =>
    {
      gl::log::error!( "Failed to create renderer: {err}" );
      return;
    }
  };
