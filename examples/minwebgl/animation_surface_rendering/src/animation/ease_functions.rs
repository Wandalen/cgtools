mod private
{
  ...
  // This module defines the complete standard named easing-curve set (the cubic-bezier
  // definitions from easings.net) as a ready-to-use palette; this example only wires up
  // `EASE_IN_OUT_BACK`, so the rest are genuinely unused from this binary's own
  // reachability graph, not accidental leftovers — kept as a deliberately complete API.
  #[ allow( dead_code ) ]
