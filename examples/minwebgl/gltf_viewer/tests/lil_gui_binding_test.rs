...
//!
//! `gltf_viewer` is a binary-only example crate (no `[lib]` target), so this test reads both
...
/// `getTitle` at all (that name only exists in older sibling crates' `gui.js` copies, e.g.
/// `area_light`/`deferred_shading`, which this file was evidently copy-pasted from). Calling
/// `name_set` as written would be a `wasm_bindgen` runtime error: no such JS export.
///
/// ## Why Not Caught
/// `name_set` is never actually called anywhere in this crate (`gui_setup.rs` only wires up the
/// bloom/exposure sliders) — the binding is currently dead code, so its runtime-error-on-call
