...
fn update() {
    ...
    gl::uniform::matrix_upload(&gl, mvp_location, mvp.raw_slice(), false).unwrap();

    prepare_vertex_attributes();
    prepare_texture_array("tileset.png", LAYERS, GL::TEXTURE0);

    let Some(ref map) = *MAP.lock().unwrap() else{
      panic!("Map is empty");
    };
    ...
    let data = map.iter().cloned().flatten().collect::<Vec<u8>>();
