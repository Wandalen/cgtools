//! Native `wgpu` backend render test : renders one real frame of the
...
//! ( `-orrery_wgpu.png`, byte-identical to `-orrery_vulkan.png` — see
//! `vulkan_render_test.rs` ), not guessed.
...
fn scene_render_produces_expected_landmarks()
{
  ...
  .expect( "no native wgpu adapter : the native backend needs a Vulkan ICD ( a software one such as lavapipe suffices )" );
