...
/// `tests/native_backend_test.rs`'s offscreen `triangle_render_readback` uses,
...
struct Renderer
{
  ...
  index_buffer : Buffer,
  /// Drawable size as of the most recent `resize`/construction. `Surface` has
  /// no `as_native_windowed()` accessor to read this back through ( unlike the
  /// Vulkan sibling example's `as_vulkan_windowed()` — see gpu_hal's src/*.rs,
  /// only the Vulkan windowed surface has a drill-down ), so this example
  /// tracks it itself from the window events it already handles.
  size : ( u32, u32 ),
}

impl Renderer
{
  /// Creates a native `wgpu` device presenting to `window`, and builds the
  /// triangle's pipeline and buffers against it.
  fn new( window : Arc< Window > ) -> Self
  {
    let inner_size = window.inner_size();
    let size = ( inner_size.width.max( 1 ), inner_size.height.max( 1 ) );
    let ( device, queue, surface ) = Device::new_native_windowed( window, size )
    .expect
    (
      "no windowed native device : this example needs a Vulkan/Metal/DX12/GL adapter \
       wgpu can build a presentable surface against for the current display server"
    );
    ...
    Self { device, queue, surface, pipeline, bind_group, uniform_buffer, vertex_buffer, index_buffer, size }
  }
  ...
  /// Reconfigures the swapchain for a new drawable size, and remembers the
  /// size for the next `SurfaceNotReady` recovery in `draw`.
  fn resize( &mut self, size : ( u32, u32 ) )
  {
    self.size = size;
    // A zero dimension is a minimized window ; the surface reports that as
    // an error and keeps its existing chain, so rendering resumes on its own
    // once the window comes back.
    let _ = self.surface.resize( size );
  }
}
...
impl ApplicationHandler for App
{
  fn resumed( &mut self, event_loop : &ActiveEventLoop )
  {
    ...
    .with_title( "gpu_hal triangle -- native wgpu swapchain" )
    .with_inner_size( winit::dpi::LogicalSize::new( INITIAL_SIZE.0, INITIAL_SIZE.1 ) );
    let window = Arc::new( event_loop.create_window( attributes ).expect( "window creation failed" ) );
