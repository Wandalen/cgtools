...
Circles are draggable: click and hold the left mouse button on a circle to pick it up. While held, the circle ignores gravity and tracks the cursor exactly (offset by the point originally grabbed), and `circles_collide` treats it as infinite mass — it pushes every other circle it touches without being pushed back itself, the same one-sided response `WallBounce` already applies at the arena walls. Releasing hands the circle back to ordinary physics with whatever velocity the last drag frames produced, so a fast flick throws it; that velocity is capped to a plausible maximum so a rendering stall mid-drag can't spike it into an unphysical launch.
