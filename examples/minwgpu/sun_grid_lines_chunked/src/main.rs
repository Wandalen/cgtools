...
fn run()
{
  ...
  image::save_buffer( "-sun_grid_lines_chunked.png", &data, width, height, image::ColorType::Rgba8 )
...
fn create_render_pipeline( device : &wgpu::Device, bind_group_layout : &wgpu::BindGroupLayout ) -> wgpu::RenderPipeline
{
  ...
  // never validated WGSL syntax either; create_shader_module below is where
  // that always happens, for both a single-file shader and a composed one.
  ...
  (
    ...
    {
      label : Some( "sun_grid_lines_pipeline" ),
