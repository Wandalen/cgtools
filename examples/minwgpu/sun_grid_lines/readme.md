...
A native-wgpu port of [`minwebgl_sun_grid_lines`](../../minwebgl/sun_grid_lines/readme.md): the same sci-fi HUD-style solar-system diagram — dark gradient background, noise-modulated nebula band, twinkling star field, grid overlay, granulated corona-wrapped star, glowing orbit ring, and one or more orbiting nodes — rendered by a single fullscreen fragment shader. It reuses the identical WGSL shader (`shaders/scene.wgsl`) written for the [browser WebGPU port](../../minwebgpu/sun_grid_lines/readme.md), byte-for-byte — WGSL is portable across both.
