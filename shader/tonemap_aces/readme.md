...
bottom half shows `tonemap_aces( vec3f( p.x ) )`, written directly as RGB
at `preview_scale = 8`. The top half slams into flat white as `p.x`
exceeds 1; the bottom half rolls off through a long filmic shoulder that
keeps gradation across the full drifting range. Directly previewable via
`sch preview tonemap_aces`.
...
| `export` | `fn tonemap_aces(color: vec3f) -> vec3f`, `fn tonemap_aces_preview(p: vec2f) -> vec3f` |
