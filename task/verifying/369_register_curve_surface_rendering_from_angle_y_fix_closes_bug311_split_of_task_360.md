# 369: Register curve_surface_rendering from_angle_y fix (closes BUG-311, split of task 360)

## Execution State

- **id:** 369
- **title:** Register curve_surface_rendering from_angle_y fix (closes BUG-311, split of task 360)
- **state:** 🔬 (Verifying)
- **open:** true
- **in_motion:** true
- **round:** 1
- **closes:** BUG-311
- **filed:** 2026-08-18 17:44:33
...
Register `curve_surface_rendering`'s already-applied, already-verified `from_angle_y`
...
task per crate -- this is that split's `curve_surface_rendering` slice. **Observable**: the call
site at `src/main.rs:179` states `gl::Quat::from_angle_y( 90.0_f32.to_radians() )`, with a 3-field
...
**Testable**: `cargo check -p curve_surface_rendering --target wasm32-unknown-unknown` -> 0
errors; `grep -c "rotation_set( gl::Quat::from_angle_y( 90.0_f32.to_radians() ) )"
examples/minwebgl/curve_surface_rendering/src/main.rs` -> 1.
...
- `lottie_surface_rendering`, `animation_surface_rendering` (own sibling split tasks -- each
...
- `module/math/ndarray_cg` and its `test_from_angle_y_rejects_raw_degrees` regression test (own
  sibling split task -- the library-level reproducer belongs to the crate that hosts it, not to
  any one example crate that happens to exercise the API it guards).
- Any further code change to this crate or to `ndarray_cg::Quat::from_angle_y` itself -- the
  library API is correct and documented as taking radians (BUG-311 Root Cause H1); not touched by
  the original fix and not touched by this task.
...
- Green state already confirmed: `cargo check -p curve_surface_rendering --target
...
- Task state reaches 🎯 only if this task file's own Readiness Verification Gate genuinely passes
  ...
  `curve_surface_rendering`, confirmed via `Cargo.toml` inspection), unlike source task 360.
...
|---|---------------|-------------------|-------------------|
| T01 | `grep -c "rotation_set( gl::Quat::from_angle_y( 90.0_f32.to_radians() ) )" examples/minwebgl/curve_surface_rendering/src/main.rs` | Fixed call site, call-site-anchored pattern (avoids double-counting the comment's own documentation text) | 1 |
| T02 | `grep -c "rotation_set( gl::Quat::from_angle_y( 90.0 ) )" examples/minwebgl/curve_surface_rendering/src/main.rs` | Naive unfixed pattern must not remain live in code (it also matches the comment's "was" clause, so a positive count here does not by itself indicate an unfixed call site) | 0 |
| T03 | `cargo check -p curve_surface_rendering --target wasm32-unknown-unknown` | Crate compiles for its target platform | 0 errors |
| T04 | `grep -c "Fix(BUG-311)" examples/minwebgl/curve_surface_rendering/src/main.rs` | Source comment present | >=1 |

## Acceptance Criteria

- `examples/minwebgl/curve_surface_rendering/src/main.rs` states `from_angle_y(
...
- That call site's source comment carries all 3 required fields: `Fix(BUG-311)`, `Root cause`,
  `Pitfall`.
- `cargo check -p curve_surface_rendering --target wasm32-unknown-unknown` -> 0 errors.
...
- [ ] C4 -- Is `module/math/ndarray_cg/src/` untouched by this task (`git diff --stat` empty for
      that path)?
- [ ] C5 -- Is `examples/minwebgl/lottie_surface_rendering/` and
      `examples/minwebgl/animation_surface_rendering/` untouched by this task (`git diff --stat`
      empty for both paths)?

### Invariants

- [ ] I1 -- workspace still builds: `cargo check -p curve_surface_rendering --target
...
| D3 | Value / YAGNI | — | 🟢 | — | — |
| D4 | Implementation Readiness | — | 🟢 | Adversarial pass specifically checked T02's grep pattern (`rotation_set( gl::Quat::from_angle_y( 90.0 ) )`) against the false-positive risk task 360's own M1/M2 measurements and task 358's D4 finding both documented for this exact comment style (`Fix(BUG-311): \`from_angle_y( 90.0 )\` -> ...` lacks the `rotation_set( gl::Quat::` wrapper) — confirmed the call-site-anchored pattern does not match the comment text, so T02's expected `0` is genuine, not a latent false-positive | — |
| D5 | Execution Scope | — | 🟢 | — | — |
| D6 | Crate Scope Unity | — | 🟢 | Single crate (`curve_surface_rendering`, confirmed via `Cargo.toml` `name =` field) — the whole point of this split from task 360 | — |
...
**Reproduced live during this gate:** `cargo check -p curve_surface_rendering --target
...
examples/minwebgl/curve_surface_rendering/src/main.rs` → 1. `grep -c "Fix(BUG-311)"` same file →
1.
...
|---------------------|----------------------|-------|--------------|
| 2026-08-18 17:44:33 | user1@w002/home/user1/pro/lib/yrd_gamedev/cgtools/ | FILED | task created |
...
| 2026-08-19 22:45:32 | user1@w002/home/user1/pro/lib/yrd_gamedev/cgtools/ | VERIFY_PASS_ATTEMPTED | `tsk .verify_pass 369` → exit 1, "self-verification forbidden (actor matches filed_by)" — same-actor sandbox guard; not forced/spoofed, left at 🔬 Verifying per standing project convention |
...
| 2026-08-20 10:03:47 | user1@w002/home/user1/pro/lib/yrd_gamedev/cgtools/ | VERIFY_PASS_ATTEMPTED | `tsk .verify_pass 369` → exit 1, same-actor guard (unchanged). Round 7 fresh-reclaim re-confirmation: BUG-311 still 🎯 Verified, cited fix-file paths still resolve (mechanical drift check clean) |
...
- **[2026-08-18]** `FILED` -- Task filed via PROC17 (`tsk.rulebook.md § Core Procedures :
  Procedure - Decompose by Crate`) to formally register task 360's `curve_surface_rendering`
  slice of BUG-311's already-applied, already-verified `from_angle_y` degrees/radians fix
  (`src/main.rs:179`) as a tracked, single-crate task -- one of 4 siblings (369/370/371/372)
...
- **[2026-08-18]** `READINESS_GATE_PASS` -- Tier 2 Dual-Role Self-Check, Round 1, 8/8 PASS, no
  ...
  verified live: `cargo check -p curve_surface_rendering --target wasm32-unknown-unknown` (via
  `longrun`, exit 0); `grep -c "rotation_set( gl::Quat::from_angle_y( 90.0_f32.to_radians() )
  )" examples/minwebgl/curve_surface_rendering/src/main.rs` → 1; `grep -c "Fix(BUG-311)"` same
  file → 1. `tsk .claim_verify 369` succeeded (❓→🔬, moved to `verifying/`).
- **[2026-08-18]** `EXECUTED` -- No new code edit performed: the described fix
  (`examples/minwebgl/curve_surface_rendering/src/main.rs:179`, `from_angle_y( 90.0 )` ->
  ...
  and lifecycle walk, not the code change itself. `tsk .verify_pass 369` blocked by the
