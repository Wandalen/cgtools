# 371: Register animation_surface_rendering from_angle_y fix (closes BUG-311, split of task 360)

## Execution State

- **id:** 371
- **title:** Register animation_surface_rendering from_angle_y fix (closes BUG-311, split of task 360)
- **state:** 🔬 (Verifying)
- **open:** true
- **in_motion:** true
- **round:** 1
- **closes:** BUG-311
- **filed:** 2026-08-18 17:47:50
...
Register `animation_surface_rendering`'s already-applied, already-verified `from_angle_y`
...
task per crate -- this is that split's `animation_surface_rendering` slice. **Observable**: the
call site at `src/main.rs:240` states `gl::Quat::from_angle_y( 90.0_f32.to_radians() )`, with a
3-field `Fix(BUG-311)`/`Root cause`/`Pitfall` source comment immediately above it (verified
present, live, 2026-08-18). **Scoped**: exactly one crate, one call site -- no other file or
crate touched. **Testable**: `cargo check -p animation_surface_rendering --target
wasm32-unknown-unknown` -> 0 errors; `grep -c "rotation_set( gl::Quat::from_angle_y(
90.0_f32.to_radians() ) )" examples/minwebgl/animation_surface_rendering/src/main.rs` -> 1.

## In Scope

- `examples/minwebgl/animation_surface_rendering/src/main.rs:234-240` -- the already-applied
...
- `curve_surface_rendering`, `lottie_surface_rendering` (own sibling split tasks -- each crate's
  task registry is self-contained per `tsk.rulebook.md`'s Cross-Crate Deliverable Note; no
  dependency edge to either, since neither appears in this crate's own `Cargo.toml`).
...
- Green state already confirmed: `cargo check -p animation_surface_rendering --target
...
- Task state reaches 🎯 only if this task file's own Readiness Verification Gate genuinely passes
  ...
  `animation_surface_rendering`, confirmed via `Cargo.toml` inspection), unlike source task 360.
...
|---|---------------|-------------------|-------------------|
| T01 | `grep -c "rotation_set( gl::Quat::from_angle_y( 90.0_f32.to_radians() ) )" examples/minwebgl/animation_surface_rendering/src/main.rs` | Fixed call site, call-site-anchored pattern | 1 |
| T02 | `grep -c "rotation_set( gl::Quat::from_angle_y( 90.0 ) )" examples/minwebgl/animation_surface_rendering/src/main.rs` | Naive unfixed pattern must not remain live in code | 0 |
| T03 | `cargo check -p animation_surface_rendering --target wasm32-unknown-unknown` | Crate compiles for its target platform | 0 errors |
| T04 | `grep -c "Fix(BUG-311)" examples/minwebgl/animation_surface_rendering/src/main.rs` | Source comment present | >=1 |

## Acceptance Criteria

- `examples/minwebgl/animation_surface_rendering/src/main.rs` states `from_angle_y(
...
- That call site's source comment carries all 3 required fields: `Fix(BUG-311)`, `Root cause`,
  `Pitfall`.
- `cargo check -p animation_surface_rendering --target wasm32-unknown-unknown` -> 0 errors.
...
- [ ] C1 -- Does `examples/minwebgl/animation_surface_rendering/src/main.rs` state `from_angle_y(
      90.0_f32.to_radians() )` (not `from_angle_y( 90.0 )`)?
...
- [ ] C5 -- Is `examples/minwebgl/curve_surface_rendering/` and
      `examples/minwebgl/lottie_surface_rendering/` untouched by this task (`git diff --stat`
      empty for both paths)?

### Invariants

- [ ] I1 -- workspace still builds: `cargo check -p animation_surface_rendering --target
...
| D5 | Execution Scope | — | 🟢 | — | — |
| D6 | Crate Scope Unity | — | 🟢 | Single crate (`animation_surface_rendering`, confirmed via `Cargo.toml` `name =` field) — the whole point of this split from task 360 | — |
...
**Reproduced live during this gate:** `cargo check -p animation_surface_rendering --target
...
examples/minwebgl/animation_surface_rendering/src/main.rs` → 1. `grep -c "Fix(BUG-311)"` same
file → 1.
...
|---------------------|----------------------|-------|--------------|
| 2026-08-18 17:47:50 | user1@w002/home/user1/pro/lib/yrd_gamedev/cgtools/ | FILED | task created |
...
| 2026-08-20 10:03:47 | user1@w002/home/user1/pro/lib/yrd_gamedev/cgtools/ | VERIFY_PASS_ATTEMPTED | `tsk .verify_pass 371` → exit 1, same-actor guard (unchanged). Round 7 fresh-reclaim re-confirmation: BUG-311 still 🎯 Verified, cited fix-file paths still resolve (mechanical drift check clean) |
...
- **[2026-08-18]** `FILED` -- Task filed via PROC17 (`tsk.rulebook.md § Core Procedures :
  Procedure - Decompose by Crate`) to formally register task 360's
  `animation_surface_rendering` slice of BUG-311's already-applied, already-verified
  `from_angle_y` degrees/radians fix (`src/main.rs:240`) as a tracked, single-crate task -- one
  of 4 siblings (369/370/371/372) splitting task 360 after its own D6 (Crate Scope Unity) FAIL,
  per the user's explicit "Yes, proceed now" authorization to run DECOMPOSE_SPLIT.
- **[2026-08-18]** `READINESS_GATE_PASS` -- Tier 2 Dual-Role Self-Check, Round 1, 8/8 PASS, no
  ...
  verified live: `cargo check -p animation_surface_rendering --target wasm32-unknown-unknown`
  (via `longrun`, exit 0); `grep -c "rotation_set( gl::Quat::from_angle_y( 90.0_f32.to_radians()
  ) )" examples/minwebgl/animation_surface_rendering/src/main.rs` → 1; `grep -c
  "Fix(BUG-311)"` same file → 1. `tsk .claim_verify 371` succeeded (❓→🔬, moved to
  `verifying/`).
- **[2026-08-18]** `EXECUTED` -- No new code edit performed: the described fix
  (`examples/minwebgl/animation_surface_rendering/src/main.rs:240`, `from_angle_y( 90.0 )` ->
  ...
  and lifecycle walk, not the code change itself. `tsk .verify_pass 371` blocked by the
