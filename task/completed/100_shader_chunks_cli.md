...
Implementation completed; acceptance walk performed as a self-administered Tier 2
Dual-Role Self-Check (repo convention — verification capped at Tier 2, never
escalated) against this file's own `## Verification` checklist. Result: **7/7
gates PASS**, one Non-Blocking Finding on I3 (see below).

**Bugs found and fixed during acceptance testing** (none were pre-existing in
`shader_chunks`/task 099 — both were local to this crate):
- `tree` rendered tag text in place of chunk names and dropped the root
  chunk from its own tree, one level short. Root cause:
  `data_fmt::TreeFormatter::format_aligned` renders a node's `ColumnData`
  (not its bare `name`), and `TreeConfig::default().show_root == false`
  skips whatever node is passed as the top-level argument. Fixed in
  `src/lib.rs`: `dep_tree()` now duplicates `name` as `ColumnData` column 0;
  `tree_chunk()` wraps each root as the sole child of an invisible,
  data-less parent node so it renders as a normal row instead of being
  skipped.
- `compose <name...>` silently produced empty output and exit 0 instead of
  composing. Root cause: `unilang` 0.59.1's positional binding for a
  `multiple: true` + `Kind::List` argument coerces each raw argv token
  against the *outer* `Kind::List` type rather than the inner `Kind::String`
  element type, double-wrapping each token into a nested one-element list
  (`List([List([String(...)]), ...])`) rather than the flat list one would
  expect — an upstream `unilang` inconsistency, not a local defect. Fixed in
  `src/main.rs` with a recursive `flatten_names()` helper that flattens
  `Value::List` of arbitrary nesting depth.
- Two clippy lints (`manual_let_else` in `cmd_get`, `collapsible_if` in
  `main`'s argv normalization) — fixed per clippy's own suggested rewrite.

**I1/I2 invariants:** `cargo test -p shader_chunks_cli --all-features` → 19/19
passed (8 subprocess + 11 direct-call), 0 failed. `cargo clippy -p
shader_chunks_cli --all-targets --all-features -- -D warnings` → 0 warnings.
Both re-confirmed after the fixes above with no regression.

**I3 invariant — Non-Blocking Finding (external, out of this task's scope):**
`cargo check --workspace --all-features` fails with 75 errors, all in
`examples/minwebgl/animation_surface_rendering` against
`interpoli::Value<T: Tween>` (E0277/E0308/E0599). Root-caused, not fixed:
`interpoli` is an unpinned git dependency (`git = "..."`, no `rev`/`tag`) and
this workspace's `Cargo.lock` is untracked, so dependency resolution is not
reproducible — an upstream `interpoli` commit apparently now conflicts with
that example's usage. Confirmed unrelated to this task: `git status
--short`/`git diff` show zero uncommitted changes to that crate or to
`interpoli`'s own usage; `git log` shows the only commit touching that
crate's directory (`bb5c07df`, made mid-session by a concurrent actor, not by
this task) changed 4 unrelated lines in its `Cargo.toml`. Scoped verification
— `cargo check --workspace --all-features --exclude
animation_surface_rendering` — exits 0, confirming the rest of the workspace,
including this crate's own integration into the workspace dependency graph,
builds clean. Fixing `animation_surface_rendering`/`interpoli` is out of this
task's Crate Locality scope (a different crate, a different dependency, no
...
"feat: add shader_chunks_cli crate and restructure module documentation"`,
which included this crate's `Cargo.toml`, `docs/cli/`, and `tests/` as they
stood at that point, plus a pre-fix snapshot of `src/lib.rs`/`src/main.rs`.
This session's subsequent fixes (above) landed as normal uncommitted
working-tree changes on top of that commit — nothing from this task's work
...
- **2026-08-12** `COMPLETED` — Acceptance walk passed (Tier 2 Dual-Role Self-Check, 7/7 gates, 1 non-blocking external finding on I3 — see Outcomes). All 5 commands verified correct against the 4 real bundled chunks; 19/19 tests passing; clippy clean; docs/cli/ and tests/docs/cli/ complete with zero TBD markers. Moved to `task/completed/`.
