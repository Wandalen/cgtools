...
| 2026-08-17 | note | Routine `task/verified/` cleanup removed the `-0056`/`-0059_longrun.log` files cited in `## Symptom` above (hyphen-prefixed, gitignored, freely-deletable per this project's storage-tier convention). No effect on this bug's record: both citations' substance (`exit 101`, identical 2-error output) is already transcribed in the surrounding `## Symptom` prose, and `## Fix Applied`'s own independent verification (three fresh exit-0 runs) does not depend on either log. |
