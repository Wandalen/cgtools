# BUG-249: `shader_chunks_query`'s `docs/cli/param/21_width.md` still claimed `width::` truncates in `table` format, contradicting BUG-116's fix (`table` now wraps)
...
|------|------|------|-----|--------|-------|
| D1 | Completeness | 🟠 | 🟢 | Confirming pass drafted all 12 FI008 sections plus Verification Record, judged complete. Adversarial pass checked `task/bug/readme.md`'s Open Bugs table directly (`grep -c "BUG-249"`) rather than assuming the row existed because BUG-115/116's did — found 0 matches: the row was genuinely missing. | Added the BUG-249 row to `task/bug/readme.md`'s Open Bugs table, matching BUG-115/116's exact column format. |
