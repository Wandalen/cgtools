...
- **Related Bugs:** [BUG-116](../unverified/116_query_table_plain_width_wraps_documentation_mismatch.md) — same `render_table` closure, same `data_fmt` 0.8.1 dependency, same `width::` parameter; discovered as a byproduct of this bug's own Step 10 docs sweep, confirmed as a distinct, pre-existing, separate defect (not caused or fixed by this bug's change)
