...
- **Fix Task:** [254](../../verifying/254_diamond_uv_buffer_stride_fix_registration.md) (renumbered from 243→252→254; two collisions same-day, bug/task ID collision with BUG-243 then a fresh race with a concurrent actor's BUG-252, 2026-08-17)
...
| 2026-08-17 | promoted to fix task | Linked to [Task 252](../../verifying/254_diamond_uv_buffer_stride_fix_registration.md) via the `bug_promote` skill (PROC12) — formal task-system registration of this bug's already-applied, already-verified fix. Task 252 (renumbered from 243 same-day, bug/task ID collision with BUG-243) reached its own Readiness Verification Gate PASS 8/8 and is blocked on `tsk .verify_pass`'s same-actor guard (identical to this bug's own filing/verifying actor), same standing pattern as this backlog's other same-actor-blocked tasks. |
| 2026-08-17 | fix task renumbered again (252 → 254) | A concurrent session actor independently filed `BUG-252` in the same real-time window this bug's own fix task landed at 252, racing both IDs onto the same number (confirmed via file mtimes: their `BUG-252` predates this task's own 243→252 rename by ~118s). Since their bug file was already terminal (`task/bug/completed/`) and cross-referenced elsewhere, the fix task moved again to 254 rather than displacing theirs. See fix task's own `## History` for full detail. |
