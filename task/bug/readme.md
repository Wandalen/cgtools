...
| BUG-109 | [`mingl::web::file::load` resolves document-relative asset paths against the origin root instead of the current page's own directory, breaking every example deployed under a subpath](./completed/109_file_load_resolves_relative_paths_against_origin_not_page_dir.md) | Critical | module/min/mingl | 2026-08-13 | 2026-08-13 | `url_resolve` joined document-relative paths against `window.location().origin()` (scheme+host+port only, no path) instead of the current page's own directory — correct only for domain-root deployments, wrong for this repo's per-example subpath deployment shape | 0 | user1@w002/home/user1/pro/lib/yrd_gamedev/cgtools/task/bug/ (self) |
