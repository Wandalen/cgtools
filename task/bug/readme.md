...
|----|-------|----------|-----------|-------|-------|
| BUG-080 | [`mingl::geometry::BoundingBox`'s `#[non_exhaustive]` breaks 7 struct-literal construction sites across 2 crates, including the default `cargo check --workspace` build](./draft/080_bounding_box_non_exhaustive_breaks_workspace_check.md) | High | module/min/mingl + module/helper/primitive_generation + examples/minwebgl/text_rendering | 2026-08-11 | 📝 Draft — fix options recorded (switch callers to `BoundingBox::new`, or revert `#[non_exhaustive]`), none applied |
