...
//! `preview`, and [`shader_chunks_render`]'s `render` — each utility also
//! ships its own standalone binary with byte-identical behavior for its
//! own commands. This crate's only responsibility is aggregation:
...
//! — the order every help screen and aggregation test pins ) and
//! handing the result to [`shader_chunks_cli_core::run`]. All command
...
mod private
{
  ...
  pub fn run()
  {
    ...
    shader_chunks_cli_core::run( shader_chunks_cli_core::CliApp
    {
      ...
      tagline : "Inspect, compose, preview, and render shader_chunks_core's bundled WGSL chunks.".to_string(),
      groups,
      examples,
      commands,
    });
