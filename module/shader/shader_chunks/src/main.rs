...
//! only wires it to `unilang`'s dispatch and argv/exit-code plumbing, routes
//! the conventional help spellings (`help`, `.`, `<command> help`) to
...
fn cli_error( err : &shader_chunks::CliError ) -> ErrorData
{
  ...
  let code = match err
  {
    shader_chunks::CliError::UnknownChunk( _ )
    | shader_chunks::CliError::UnknownField( _ )
    | shader_chunks::CliError::InvalidParam { .. }
    | shader_chunks::CliError::Compose( _ ) => ErrorCode::ValidationRuleFailed,
...
fn query_arguments( names_optional : bool, defaults : &shader_chunks::QueryParams ) -> Vec< ArgumentDefinition >
{
  vec!
  [
    ArgumentDefinition::former()
...
/// as [`shader_chunks::CliError::InvalidParam`] rather than wrapping.
...
/// Builds [`shader_chunks::QueryParams`] from a verified command's bound
...
(
  cmd : &VerifiedCommand,
  mut params : shader_chunks::QueryParams,
) -> Result< shader_chunks::QueryParams, ErrorData >
{
...
/// exactly this function over [`shader_chunks::query_chunks`], differing
/// only in the `defaults` constructor passed in.
fn query_routine( defaults : fn() -> shader_chunks::QueryParams ) -> CommandRoutine
{
  Box::new( move | cmd, _ctx |
  {
    let params = query_params_from( &cmd, defaults() )?;
    let content = shader_chunks::query_chunks( &params ).map_err( | e | cli_error( &e ) )?;
...
{
  let defaults = shader_chunks::QueryParams::list_defaults();
  ...
  ( def, query_routine( shader_chunks::QueryParams::list_defaults ) )
}

fn cmd_get() -> ( CommandDefinition, CommandRoutine )
{
  let defaults = shader_chunks::QueryParams::get_defaults();
  let def = CommandDefinition::former()
  .name( ".get" )
  .namespace( String::new() )
  ...
  ( def, query_routine( shader_chunks::QueryParams::get_defaults ) )
}
...
fn cmd_compose() -> ( CommandDefinition, CommandRoutine )
{
  ...
  .deprecation_message( String::new() )
  .http_method_hint( String::new() )
  .examples( vec![ "shader_chunks compose hash21 value_noise".to_string() ] )
  .arguments( vec!
  [
    ArgumentDefinition::former()
    .name( "names" )
