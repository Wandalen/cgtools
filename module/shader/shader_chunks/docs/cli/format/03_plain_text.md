...
| Rendering mechanism | Hand-built `format!` string in `src/lib.rs` (`get_chunk`, deferring to `shader_chunks_core::try_compose` for `compose`) — no `data_fmt` table/tree pipeline, since neither output is tabular or hierarchical |
| Example | See below |
