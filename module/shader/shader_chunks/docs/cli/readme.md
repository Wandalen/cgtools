...
9 commands for querying, inspecting, composing, previewing, rendering,
validating, and introspecting the tunable parameters of the WGSL shader
chunks bundled by
...
[`shader_chunks_validate_core`](../../../shader_chunks_validate_core/readme.md)+[`shader_chunks_validate`](../../../shader_chunks_validate/readme.md)),
...
21-parameter filter/projection/formatting surface; `tree` renders the
...
static PNG; `validate` runs five registry-wide integrity checks —
manifest drift, duplicate names, missing dependencies, dependency
cycles, WGSL compilation — and reports every finding in one pass via
[`shader_chunks_validate`](../../../shader_chunks_validate/readme.md).
...
|------------|----------|--------|------------|-----------|-------------|------|
| [`shader_chunks_query`](../../../shader_chunks_query/docs/cli/readme.md) | `list`, `get`, `tags`, `tree` | Query, Graph | 24 | 11 | 9 | Complete |
| [`shader_chunks_compose`](../../../shader_chunks_compose/docs/cli/readme.md) | `compose` | Compose | 1 (`out`; also reuses `names`/`transitive`) | 0 | 1 | Complete |
...
| [`shader_chunks_render`](../../../shader_chunks_render/docs/cli/readme.md) | `render` | Render | 5 | 2 | 0 (reuses `plain_text`) | Complete |
| [`shader_chunks_validate`](../../../shader_chunks_validate/docs/cli/readme.md) | `validate` | Validate | 0 (declares none) | 0 | 0 (reuses `plain_text`) | Complete |
| **Total** | **9** | **7** | **32** | **13** | **10** | |
...
6 leaf crates for this 9-command, single-domain-object CLI; no
...
`shader_chunks_render`, `shader_chunks_validate_core`+
`shader_chunks_validate`), each with its own `tests/`. Each leaf's own
...
- [`shader_chunks_compose/docs/cli/`](../../../shader_chunks_compose/docs/cli/readme.md) — `compose`; the `out` param, the `plain_text` format
...
- [`shader_chunks_render/docs/cli/`](../../../shader_chunks_render/docs/cli/readme.md) — `render`; `out`/`size`/`time`/`set`/`all` params, the `Float`/`ParameterOverride` types
- [`shader_chunks_validate/docs/cli/`](../../../shader_chunks_validate/docs/cli/readme.md) — `validate`; zero params, reuses the `plain_text` format
...
`cli_doc_des.rulebook.md`, made because this CLI is 9 inspection and
...
- **`user_story/` omitted.** The rulebook requires a `### Referenced User
  ...
  developer inspects/composes/previews/renders/validates bundled shader
  chunks from the terminal," one story stretched thin across 9 commands,
  not 9 genuine personas/goals. Below the rulebook's own 5-instance
  guidance for the collection to be worthwhile. Every file that formally
  requires the section still carries it, pointing back to this paragraph
  instead of a fabricated story.
...
- **`param_group/` included** (formerly omitted). The omission rationale —
  ...
  share 20 named parameters verbatim, partitioned into 3 groups.
...
- **`command/`, `command_group/`, `param/`, `param_group/`, `type/`, and
  ...
  `_validate` — see the Family Completion Matrix and Navigation above).
  This aggregator crate contains no command logic of its own — every
  command, parameter, type, and format is owned and tested inside its
  implementing crate —
