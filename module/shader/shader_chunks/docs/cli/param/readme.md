...
- **In Scope:** The 21 parameters declared across the 6 commands' `ArgumentDefinition`s — 2 positional selectors plus the 19 named query parameters `.list` and `.get` share verbatim (one of which, `transitive::`, `.compose` also accepts as its closure switch).
- **Out of Scope:** Command-level syntax/examples (→ [`../command/`](../command/readme.md)), type constraints/parsing (→ [`../type/`](../type/readme.md)), co-occurrence/interaction rules (→ [`../param_group/`](../param_group/readme.md)).
...
|---|------|-----------|------|---------|--------|
| 1 | [01_name.md](01_name.md) | `name` | [`ChunkName`](../type/01_chunk_name.md) | `Varies` — optional for `tree` (omit for the forest), required for `tunables` | ✅ |
