...
| ID | Case | Real Test |
|----|------|-----------|
| EC-1 | Valid chunk — direct dependents selected | `shader_chunks_test.rs::query_depends_on_selects_direct_dependents_and_transitive_widens` |
| EC-2 | Same chunk with `transitive::1` — closure widens the set | `shader_chunks_test.rs::query_depends_on_selects_direct_dependents_and_transitive_widens` |
| EC-3 | Unknown chunk fails loudly (`CliError::UnknownChunk`) | `shader_chunks_test.rs::query_depends_on_unknown_chunk_fails_loudly` |
