...
| ID | Case | Real Test |
|----|------|-----------|
| EC-1 | `any` unions the same selector list `all` intersects | `shader_chunks_test.rs::query_tags_mode_any_unions_and_all_intersects_selectors` |
| EC-2 | Round-trip of both spellings; bogus value rejects loudly | `shader_chunks_test.rs::query_enum_params_round_trip_and_reject_bogus_values` |
