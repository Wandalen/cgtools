...
| ID | Case | Real Test |
|----|------|-----------|
| TC-1 | Round-trip: all 4 variants survive `as_str` → `parse` | `shader_chunks_test.rs::query_enum_params_round_trip_and_reject_bogus_values` |
...
| TC-3 | Behavioral: each variant produces its documented deterministic ordering | `shader_chunks_test.rs::query_sort_keys_order_deterministically` |
