...
| ID | Case | Real Test |
|----|------|-----------|
| TC-1 | Bare form (`tag`): matches the tag under any group | `shader_chunks_test.rs::query_bare_tag_selector_matches_the_tag_under_any_group` |
| TC-2 | Pair form (`group:tag`, via `split_once(':')`): demands the exact group | `shader_chunks_test.rs::query_pair_tag_selector_demands_the_exact_group` |
| TC-3 | Multiple selectors combine per `tags_mode::` | `shader_chunks_test.rs::query_tags_mode_any_unions_and_all_intersects_selectors` |
