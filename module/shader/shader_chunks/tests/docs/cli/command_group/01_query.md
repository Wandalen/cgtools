...
| CG-4 | Idempotent, no side effects, registry-only — output is a pure function of arguments | Structural: `query_chunks` and `list_tags` in `src/lib.rs` take `&QueryParams`/no input and return fresh `Result<String, CliError>`; no `std::fs`/`std::env` usage (mechanically re-checkable via `grep -rn "std::fs\|std::env" src/`) |
