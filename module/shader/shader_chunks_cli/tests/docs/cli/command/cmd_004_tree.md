...
| ID | Case | Real Test |
|----|------|-----------|
| PAR-1 | Valid, known chunk name (`fbm3`) — shows `fbm3 -> value_noise -> hash21` | `shader_chunks_cli_test.rs::tree_chunk_shows_fbm3_dependency_chain_in_order`; `cli_subprocess_test.rs::tree_fbm3_shows_the_dependency_chain` |
| PAR-2 | Absent (omitted entirely) — shows the full forest | `shader_chunks_cli_test.rs::tree_chunk_with_no_name_shows_forest_of_every_root_chunk` |
| PAR-3 | Unknown chunk name | `shader_chunks_cli_test.rs::tree_chunk_reports_unknown_chunk_error_for_bogus_name` |
