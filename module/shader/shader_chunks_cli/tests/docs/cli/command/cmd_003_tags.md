...
|----|----------|-----------|
| INT-1 | Direct call lists every distinct `group:tag` pair and its carrying chunk(s) | `shader_chunks_cli_test.rs::list_tags_lists_every_distinct_group_tag_pair_and_its_chunks` |
| INT-2 | Subprocess invocation prints every distinct tag | `cli_subprocess_test.rs::tags_prints_every_distinct_tag` |

See also [`001_chunk.md`](001_chunk.md) INT-3 for `tags`'s role preceding
`get` in a discover-by-tag-then-inspect workflow.
