//! Direct-call tests for `shader_chunks_cli`'s command logic — no
//! subprocess; see `tests/cli_subprocess_test.rs` for end-to-end argv and
//! exit-code coverage.

use shader_chunks_cli::{ CliError, compose_chunks, get_chunk, list_chunks, list_tags, tree_chunk, try_compose_wgsl };
...
fn compose_chunks_reports_missing_dependency_error_when_hash21_is_omitted()
{
  ...
  assert!
  (
    matches!( &err, CliError::Compose( shader_chunks::ComposeError::MissingDependency { .. } ) ),
...
fn try_compose_wgsl_reports_cyclic_dependency_error_on_synthetic_fixture()
{
  ...
  assert!
  (
    matches!( &err, CliError::Compose( shader_chunks::ComposeError::CyclicDependency( _ ) ) ),
