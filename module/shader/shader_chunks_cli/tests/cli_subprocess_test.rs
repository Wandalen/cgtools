//! Subprocess tests for the `shader_chunks_cli` binary — real process
//! spawns via `assert_cmd`, exercising the actual argv/exit-code path
//! `main.rs` implements (`tests/shader_chunks_cli_test.rs` covers the
...
{
  Command::cargo_bin( "shader_chunks_cli" )
  .expect( "shader_chunks_cli binary should build" )
  .args( args )
  .output()
  .expect( "shader_chunks_cli should spawn and run to completion" )
}
...
fn no_arguments_prints_help_and_exits_zero()
{
  ...
  assert!( output.status.success(), "stderr: {}", String::from_utf8_lossy( &output.stderr ) );
  let stdout = stdout_of( &output );
  assert!( stdout.contains( "shader_chunks_cli" ), "{stdout}" );
