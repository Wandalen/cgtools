...
- **Constraints:** Every element must resolve against
  `shader_chunks::ALL_CHUNKS` via `shader_chunks::parse_name` — the first
  ...
  `shader_chunks::try_compose`, not a panic
...
- **Purpose:** The set of chunks to compose into one WGSL preview, in any
  input order — `shader_chunks::try_compose` performs dependency-ordered
...
|------|------|-------------|----------------|
| [ChunkName](../type/01_chunk_name.md) | String (list) | `Vec<String>` | Every element must resolve in `shader_chunks::ALL_CHUNKS` |
