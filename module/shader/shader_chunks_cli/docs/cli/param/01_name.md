...
- **Constraints:** Must resolve against `shader_chunks::ALL_CHUNKS` via
  `shader_chunks::parse_name` — an unresolved name returns
...
### Notes
- Positional, not `key::value` — `shader_chunks_cli` follows unilang's
...
|------|------|-------------|----------------|
| [ChunkName](../type/01_chunk_name.md) | String | `String` | Must resolve in `shader_chunks::ALL_CHUNKS` |
