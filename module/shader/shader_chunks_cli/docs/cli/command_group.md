...
(`shader_chunks::ALL_CHUNKS`). None mutate state, none write files, none
...
`shader_chunks::try_compose` into a real render pipeline.
...
- Every command operates only on `shader_chunks::ALL_CHUNKS` — no
