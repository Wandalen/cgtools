...
- **`range(min, max)`**: optional. When present, `min`/`max` are parsed as `f64` and always take precedence over inference (see [`algorithm/001`](../algorithm/001_range_inference_heuristic.md)). When absent, the line's range is resolved by `range_infer` at discovery time.
...
| `texture` | `Texture` | A bound texture. `range_infer` always returns `None` for this kind, regardless of `<type>` or `<name>` — see [`algorithm/001`](../algorithm/001_range_inference_heuristic.md). |
...
| `RangeSource` | 2-variant enum (`Declared`, `Inferred`) | Whether a `Parameter`'s range came from the line's own `range(min, max)` clause or from `range_infer`. |
| `Range` | `{ min: f64, max: f64 }` | An inclusive numeric range. |
| `Parameter` | `{ name: String, kind: ParameterKind, value_type: ValueType, range: Option<(Range, RangeSource)> }` | One fully-parsed `//@ param:` line. `range` is `None` only when the line declared none AND `range_infer` also returned `None` (a `texture` kind or `bool` type with no declared range). |
...
| `chunk_discover` | `(chunk: &shader_chunks_core::ChunkDescriptor) -> Vec<Parameter>` | Equivalent to `discover(chunk.wgsl)` — this crate's only dependency on `shader_chunks_core`; `discover` itself has none. Same panic contract as `discover`. |
| `range_infer` | `(kind: ParameterKind, value_type: ValueType, name: &str) -> Option<Range>` | The range-resolution heuristic `discover` calls when a line declares no `range(min, max)` clause. Full rule table in [`algorithm/001`](../algorithm/001_range_inference_heuristic.md). Never panics. |
...
The crate is pre-1.0 (`0.1.0` in `Cargo.toml`); no formal deprecation policy or versioning strategy is asserted here beyond Cargo semver. Both in-workspace consumers (`shader_chunks_params`, `shader_chunks_preview_core`) depend on it via a `path`/`workspace` dependency, not a published version, so any breaking change is caught at the next workspace-wide build rather than negotiated through version ranges. The 5-variant `ParameterKind` and 14-variant `ValueType` enums are the most change-sensitive surface: a new variant is additive for callers that only construct/inspect `Parameter` values, but breaking for any downstream `match` that is already exhaustive over either enum.

### Sources

| File | Relationship |
|------|--------------|
| `src/lib.rs` | `ParameterKind`, `ValueType`, `RangeSource`, `Range`, `Parameter`, `discover`, `chunk_discover` — the entire public API this document describes |

### Tests

| File | Relationship |
|------|--------------|
| `tests/discovery_test.rs` | Exercises every kind, declared vs. inferred ranges, file-order multi-param handling, the empty-`Vec` case, every panic path, and the `chunk_discover` wrapper |
