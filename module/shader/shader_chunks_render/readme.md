...
```text
render <name> | file::<path>   [out::<path>] [size::<n>|<w>x<h>] [time::<seconds>]
  ...
  -> shader_chunks_render_core::render         # one headless frame, params at defaults
...
touches a slider — every bundle parameter takes its initial value, and
`time::` freezes the preview's time uniform at one instant (default `0`).
...
| `docs/` | CLI documentation — see [`docs/cli/`](docs/cli/readme.md) for the `render` command, `out`/`size`/`time` params, and the `Float` type |
...
let summary = render_to_png( &PreviewTarget::Name( "fbm3".to_string() ), ( 64, 64 ), 0.0, &out ).unwrap();
