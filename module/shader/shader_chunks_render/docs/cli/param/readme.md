...
- **In Scope:** The 3 parameters this crate introduces —
  `out`, `size`, `time`.
- **Out of Scope:** Command-level syntax/examples (→ [`../command/`](../command/readme.md)),
  ...
  (owned by `shader_chunks_preview`) — and the family's other 21
...
**Total:** 3 parameters owned by this crate (of 26 across the
`shader_chunks` family; `render` accepts 5 total — these 3 plus the
shared `name`/`file` target pair).

**Co-occurrence note:** these 3 parameters belong to no
