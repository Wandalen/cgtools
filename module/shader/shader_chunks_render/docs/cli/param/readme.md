...
- **In Scope:** The 5 parameters this crate introduces —
  `out`, `size`, `time`, `set`, `all`.
- **Out of Scope:** Command-level syntax/examples (→ [`../command/`](../command/readme.md)),
  ...
  (owned by `shader_chunks_preview`) — and the family's other 23
...
| 5 | [05_all.md](05_all.md) | `all` | [`Switch`](../../../../shader_chunks_query/docs/cli/type/07_switch.md) | `false` | ✅ |

**Total:** 5 parameters owned by this crate (of 32 across the
`shader_chunks` family; `render` accepts 7 total — these 5 plus the
shared `name`/`file` target pair).

**Co-occurrence note:** these 5 parameters belong to no
