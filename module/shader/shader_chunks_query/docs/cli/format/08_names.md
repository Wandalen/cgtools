...
| Rendering mechanism | Direct join of the result's `name` fields in `chunks_render` (`shader_chunks_query_core/src/lib.rs`) — no `data_fmt` pipeline needed for a bare line list |
| Example | See below |
