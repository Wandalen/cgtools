...
**Fundamental Type:** `enum TagsMode { Any, All }` in `shader_chunks_query_core/src/lib.rs` —
...
- `as_str() -> &'static str` — canonical spelling, round-trips with
  `from_str`
- Semantics (`chunk_matches` in `shader_chunks_query_core/src/lib.rs`): `Any` keeps a chunk when
