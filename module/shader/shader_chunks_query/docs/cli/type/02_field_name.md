...
is the `pub const QUERY_FIELDS : &[&str]` slice in `shader_chunks_query_core/src/lib.rs`, and
...
**Parsing:** `chunks_query` (`shader_chunks_query_core/src/lib.rs`) checks every requested field
...
**Methods:**
- `field_value(chunk, field) -> String` (`shader_chunks_query_core/src/lib.rs`) — renders the
