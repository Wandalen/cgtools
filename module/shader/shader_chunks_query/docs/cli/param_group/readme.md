...
- **Purpose:** Documents every set of co-occurring parameters shared
...
in their own crates) is outside the query engine entirely — neither
belongs to a group, nor do the preview/render-only parameters (`file`,
`serve`, `out`, `size`, `time`), which are scoped to their own crates.
