...
| ID | Case | Real Test |
|----|------|-----------|
| EC-1 | Projection keeps only the named columns, in the given order | `shader_chunks_query_core/tests/shader_chunks_query_core_test.rs::query_fields_projects_only_the_named_columns` |
| EC-2 | Every declared field renders — including multi-line `source` | `shader_chunks_query_core/tests/shader_chunks_query_core_test.rs::query_every_declared_field_renders_including_source` |
| EC-3 | Unknown field fails loudly (`CliError::UnknownField`) | `shader_chunks_query_core/tests/shader_chunks_query_core_test.rs::query_unknown_field_fails_loudly`; `cli_subprocess_test.rs::invalid_param_values_exit_non_zero_loudly` |
