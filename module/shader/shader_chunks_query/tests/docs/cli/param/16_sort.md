...
| ID | Case | Real Test |
|----|------|-----------|
| EC-1 | Each key (`input`/`name`/`stage`/`description`) orders deterministically | `shader_chunks_query_core/tests/shader_chunks_query_core_test.rs::query_sort_keys_order_deterministically` |
| EC-2 | All four values round-trip through `parse`/`as_str` | `shader_chunks_query_core/tests/shader_chunks_query_core_test.rs::query_enum_params_round_trip_and_reject_bogus_values` |
