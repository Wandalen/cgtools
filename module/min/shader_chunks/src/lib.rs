//! Manifest-driven WGSL shader-chunk composer. Each bundled chunk under
//! `src/chunks/` is exactly one WGSL function (or, for the vertex stage,
...
mod private
{
  ...
  pub const HASH21 : &str = include_str!( "chunks/hash21.wgsl" );
  /// `value_noise` chunk: bilinear value noise over [`HASH21`].
  pub const VALUE_NOISE : &str = include_str!( "chunks/value_noise.wgsl" );
  /// `fbm3` chunk: three-octave fractal Brownian motion over [`VALUE_NOISE`].
  pub const FBM3 : &str = include_str!( "chunks/fbm3.wgsl" );
  /// `fullscreen_triangle` chunk: vertex entry point emitting one screen-covering triangle.
  pub const FULLSCREEN_TRIANGLE : &str = include_str!( "chunks/fullscreen_triangle.wgsl" );
  ...
  // `export`/`stage` stay human- and future-tool-facing, unread by the
  // composer itself — this crate's tests ( `tests/shader_chunks_test.rs` )
  // cross-check `export` against the real WGSL body anyway, so it can't
  // silently drift.
