...
//! (see [`crate::error::ValidationError::IllegalSourceNesting`]).
...
mod private
{
  ...
  use crate::resource::{ SpriteRef, AnimationRef };
  ...
  pub enum SpriteSource
  {
    ...
    /// Plays a declared animation (see [`crate::resource::Animation`]). SPEC §5.3.
...
{
  exposed use SpriteSource;
  exposed use Variant;
  exposed use VariantSelection;
  exposed use Condition;
  exposed use NeighborBitmaskSource;
  exposed use AutotileLayout;
  exposed use EdgeConnectedLayout;
  exposed use TriBlendPattern;
  exposed use ViewportTiling;
  exposed use ViewportAnchorPoint;
}
