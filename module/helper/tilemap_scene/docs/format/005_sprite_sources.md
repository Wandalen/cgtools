...
| `VertexCorners` | `Vertex` only | `patterns: Vec<{corners: (ObjectId|"*", ...), sprite_pattern: String ({rot} template), priority}>`, `asset`, `orient_to_grid: bool` (default `false`), `corner_source: Option<String>` (default `None`), `offset: Option<(f32, f32)>` (default `None`) | Reads the object ids at the vertex's corners — via `corner_source` when set, otherwise the terrain channel — builds the canonical sorted tuple + rotation (see `format/003`), matches against `patterns` by specificity then priority then declaration order (see Rule Resolution below), emits with `{rot}` substituted (or a pre-baked oriented frame when `orient_to_grid` is set — see below), or emits nothing if no pattern matches. `"*"` matches any single corner and sorts as lexicographically greater than any concrete id. |
...
**`VertexCorners.corner_source`** (default `None`) selects which per-cell "channel" the corner ids are read from: `None` reads the cell's terrain id — the id of the first object on the cell that has a `priority` field set, i.e. the same rule `invariant/001` documents for the default channel. `Some("layer_name")` instead reads the id of the first object on the cell whose `global_layer` matches `layer_name`, letting multiple independent dual grids share one scene — e.g. a base terrain layer (`corner_source: None`) and a per-player region layer (`corner_source: Some("region")`) coexist on the same cells with isolated corner lookups. In both cases "first" means the cell's object-list order (`tiles[].objects` declaration order, or insertion order for a programmatically-assembled scene) — spec authors SHOULD NOT rely on this ordering to break ties between two objects that both qualify for the same channel; give such objects distinct channels instead. If no object on the cell matches the named channel, the corner silently resolves to `VOID_ID`, identical to an off-map corner — a misspelled `corner_source` therefore renders the whole layer as its void/edge patterns (often nothing) with no error.

**`VertexCorners.orient_to_grid`** (default `false`) selects a pre-baked oriented frame keyed to the triangle's discrete orientation on the hex dual grid (one of 6 sixty-degree rotations) instead of the canonical-sort rotation used when `false`. `{rot}` ranges `0..2` when the triangle is fully symmetric — all three corners equal the layer's self id (solid interior), or none do (degenerate void) — and `0..6` otherwise (an edge: two corners present, or a corner: one present). Symmetry is decided by counting corners equal to the self id, not by lexical equality of the three ids, so a cross-region boundary like `(region_1, region_1, region_0)` is correctly treated as a 6-frame edge for `region_1` even though its ids are not all equal. An atlas baker MUST order `{rot}` frames to match this selection: orientation 0's base angle is 60° for a corner tile (aligning the lone present corner) or a full tile (2-frame ▲/▽ parity only), and 300° for an edge tile (aligning the single absent corner); `{rot}` increments clockwise in world space (the baker's counter-clockwise `u_rot` reads as clockwise because the PNG export flips vertically); the reference corner for frame `rot` points at `base − 60°·rot`. Out of scope for 0.2.0: a triangle whose three corners hold three distinct non-void ids has no present/absent split, so the emitted frame is deterministic but not geometrically meaningful — atlas authors should suppress such triangles with a more-specific pattern rather than rely on the oriented frame.

**`VertexCorners.offset`** (default `None`) is a static `(dx, dy)` world-pixel shift added to every sprite this source emits — e.g. a grey-tinted copy nudged downward as a 2.5D drop-shadow under the main terrain layer. Corner resolution and `orient_to_grid` frame selection always use the true, un-shifted triangle geometry; only the final emitted sprite position shifts.

### Version Compatibility

New leaf or composite variants are expected to be additive; so are new optional fields on an existing variant — `VertexCorners.orient_to_grid`/`corner_source`/`offset` are one such already-shipped addition, all three defaulting to their 0.1.0 behaviour when omitted. The leaf/composite nesting restriction ("composite sources cannot nest inside composite sources") is enforced by `validate.rs`'s composite-nesting check — a spec author who nests a composite inside another composite's per-mask slot gets a `ValidationError::IllegalSourceNesting` at `load()` time. `format/003`'s anchor↔source applicability table (`NeighborBitmask`/`NeighborCondition`/`VertexCorners` restricted to `Hex`/`Vertex`, `EdgeConnectedBitmask` to `Edge`, all neighbour-dependent sources excluded from `Multihex`/`FreePos`/`Viewport`, `External` claimed valid on every anchor) is, by contrast, still declared but not yet checked at load time — see `pitfall/001` and `invariant/001`.
