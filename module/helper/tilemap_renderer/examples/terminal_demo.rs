...
fn main() -> Result< (), Box< dyn std::error::Error > >
{
  ...
  renderer.initialize().map_err( |e| format!( "Initialization failed: {:?}", e ) )?;
  ...
  let border_color = StrokeStyle
  {
    ...
    width: 1.0,
    cap: tilemap_renderer::commands::LineCap::Butt,
    join: tilemap_renderer::commands::LineJoin::Miter,
    miter_limit: 4.0,
  };
  
  // Top and bottom borders
  scene.add_line( Point2D { x: 5.0, y: 4.0 }, Point2D { x: 55.0, y: 4.0 }, border_color );
  scene.add_line( Point2D { x: 5.0, y: 20.0 }, Point2D { x: 55.0, y: 20.0 }, border_color );
  
  // Left and right borders
  scene.add_line( Point2D { x: 5.0, y: 4.0 }, Point2D { x: 5.0, y: 20.0 }, border_color );
  scene.add_line( Point2D { x: 55.0, y: 4.0 }, Point2D { x: 55.0, y: 20.0 }, border_color );
  
  // Add some diagonal lines for visual interest
  scene.add_line( 
    Point2D { x: 10.0, y: 8.0 }, 
    Point2D { x: 25.0, y: 16.0 }, 
    StrokeStyle { color: [ 1.0, 0.0, 0.0, 1.0 ], width: 1.0, cap: tilemap_renderer::commands::LineCap::Butt, join: tilemap_renderer::commands::LineJoin::Miter, miter_limit: 4.0 }
  );
  
  scene.add_line( 
    Point2D { x: 35.0, y: 8.0 }, 
    Point2D { x: 50.0, y: 16.0 }, 
    StrokeStyle { color: [ 0.0, 0.0, 1.0, 1.0 ], width: 1.0, cap: tilemap_renderer::commands::LineCap::Butt, join: tilemap_renderer::commands::LineJoin::Miter, miter_limit: 4.0 }
  );
  
  // Add a curved line
  scene.add_curve(
    Point2D { x: 10.0, y: 18.0 },
    Point2D { x: 30.0, y: 10.0 },
    Point2D { x: 50.0, y: 18.0 },
    StrokeStyle { color: [ 1.0, 0.0, 1.0, 1.0 ], width: 1.0, cap: tilemap_renderer::commands::LineCap::Butt, join: tilemap_renderer::commands::LineJoin::Miter, miter_limit: 4.0 }
  );
  ...
  // Output to console
  renderer.output( "" ).map_err( |e| format!( "Output failed: {:?}", e ) )?;
  
  println!( "\n" );
  
  // Also save to file for inspection
  renderer.export_to_file( "-terminal_demo_output.txt" ).map_err( |e| format!( "Export failed: {:?}", e ) )?;
  ...
  ascii_renderer.initialize().map_err( |e| format!( "ASCII initialization failed: {:?}", e ) )?;
  
  let context = RenderContext { frame_id: 2, timestamp_ms: 100 };
  ...
  ascii_scene.add_text(
    ascii_title,
    Point2D { x: 23.0, y: 2.0 },
    FontStyle
    {
  ...
  ascii_scene.add_line( Point2D { x: 10.0, y: 5.0 }, Point2D { x: 50.0, y: 5.0 }, ascii_style );
  ascii_scene.add_line( Point2D { x: 10.0, y: 10.0 }, Point2D { x: 50.0, y: 10.0 }, ascii_style );
  ascii_scene.add_line( Point2D { x: 10.0, y: 5.0 }, Point2D { x: 10.0, y: 10.0 }, ascii_style );
  ascii_scene.add_line( Point2D { x: 50.0, y: 5.0 }, Point2D { x: 50.0, y: 10.0 }, ascii_style );
  ...
  ascii_renderer.output( "" ).map_err( |e| format!( "ASCII output failed: {:?}", e ) )?;
