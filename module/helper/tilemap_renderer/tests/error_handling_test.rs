...
fn test_memory_stress_handling()
{
  ...
  for _i in 0..10  // Reduced from higher number to avoid CI issues
  {
  ...
  assert_eq!( renderers.len(), 10 );
  
  // Test that they can be initialized (may fail due to resource limits, but shouldn't panic)
  let context = RenderContext::default();
  
  for renderer in &mut renderers
  {
    let _result = renderer.initialize( &context );
    // May succeed or fail depending on system resources, but should not panic
  }
