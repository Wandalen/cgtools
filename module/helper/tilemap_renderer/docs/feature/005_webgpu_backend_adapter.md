...
The pixel-upload gap is now closed at the source level (task 218) — verified via the shared `to_rgba8` helper's own unit tests, a source-level confirmation the `1x1` placeholder literal is gone, and a live browser pixel readback matching [`NativeBackend`](006_native_backend_adapter.md)'s own (task 251, confirmed 2026-08-18) — status is tracked as complete (✅) for this adapter's declared (Sprite-only) scope.
