...
mod private
{
  ...
  use crate::scene_model::compile::neighbors::{ VOID_ID, tile_terrain_id };
  use crate::scene_model::pipeline::TilingStrategy;
  use crate::scene_model::scene::Tile;
  use crate::scene_model::source::TriBlendPattern;
  ...
  {
    let mut seen : HashSet< TriCoordinate< TO > > = HashSet::new();
    let mut out = Vec::new();
  ...
  (
    tri : &TriangleContext,
    tile_lookup : &std::collections::HashMap< ( i32, i32 ), &Tile >,
    spec : &RenderSpec,
  ) -> [ String; 3 ]
  {
    tri.corners.map( | pos |
...
{
  own use TriangleContext;
  own use enumerate_triangles;
  own use resolve_corners;
  own use canonicalize;
  own use find_matching_pattern;
}
