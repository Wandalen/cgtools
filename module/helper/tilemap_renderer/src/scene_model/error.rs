...
mod private
{
  ...
  /// Error returned by [`crate::scene_model::spec::RenderSpec::load`] /
  /// [`crate::scene_model::scene::Scene::load`] and their `from_ron_str`
  /// counterparts.
  ///
  ...
  pub enum ValidationError
  {
    ...
    /// The object's `default_animation` is not present in its `animations` map.
    MissingDefaultAnimation
    {
      /// Owning object's id.
      object : String,
      /// The animation name that was set as default but not declared.
      animation : String,
    },
  ...
  impl fmt::Display for ValidationError
  {
    ...
    fn fmt( &self, f : &mut fmt::Formatter< '_ > ) -> fmt::Result
    {
      match self
      {
        ...
        Self::MissingDefaultAnimation { object, animation } =>
          write!( f, "object {object:?} declares default_animation {animation:?} but has no such entry" ),
