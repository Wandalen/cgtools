...
- **Compressed glTF**: the loader reads `EXT_meshopt_compression` / `KHR_meshopt_compression` and `KHR_mesh_quantization`. Compressed buffer views are decoded with meshoptimizer's own decoder (vendored as `src/webgl/loaders/meshopt_decoder.js`, MIT, about 29 KB with its wasm inlined) into their fallback buffer, so the rest of the loader sees plain glTF. Quantized attributes keep their integer type on the GPU. Normalized `POSITION` bounds are dequantized (`position_bounds_dequantize`) so bounding boxes are in shader units, not raw integers. The `gltf` crate's validation refuses these `extensionsRequired` entries, so `load` now runs that validation itself (`document_validate`) and leaves the extension decision to `required_extensions_check`. Buffers are now loaded strictly by index; a buffer without a uri that is not the GLB chunk is an error instead of shifting every later buffer. Buffers are copied into wasm memory only when skins, morph targets or animations need them on the CPU.
