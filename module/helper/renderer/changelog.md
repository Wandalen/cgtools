...
- **USD browser feed + viewer USD mode ( N3 fourth slice, first browser run pending )**: `loaders::usd::usd_scene_load_http` - async fetch of a `.usda` root via `gl::file::load`, `.mtlx` reference discovery with the N3-lite `usda_mtlx_references` scanner, sibling fetches resolved against the scene folder, all fed to the new `usd_scene_from_texts` ( synchronous, in-memory-text entry point ). `.mtlx` bytes deliberately never enter the stage's own `ar::Resolver` ( MaterialX is not a USD layer; composition would try to parse it as USDA ) - they ride the separate `UsdAssetProvider` store, mirroring the native-test setup. `gltf_viewer` gained a **USD scene** mode : a procedural icosphere serialized to `.usda` text + the selected embedded `.mtlx`, through the full `loaders::usd` pipeline under the studio rig. All native tests still pass; this is the slice that finally makes the GL half user-verifiable in the browser ( `action/run gltf_viewer` ).
