...
| kulla_conty_test.rs | Tests the Kulla–Conty `E(μ,α)` / `E_avg(α)` table generator — bounds, energy conservation, roughness monotonicity — and the per-bounce Fresnel series `F_ms` weighting it ( mirror / black limits, never exceeding the single-bounce weight, tint saturation ) |
