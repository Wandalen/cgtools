...
| usd_scene_test.rs | Tests the `loaders::usd` N3 core — in-memory `ar::Resolver` stage open, fan triangulation, primvar corner resolution ( vertex / faceVarying / indexed ), `Xform` world-matrix composition, malformed-topology error paths, and the pure `usd_scene_analyze` scene pass ( hierarchy, shading-subtree skip, material binding + inheritance, visibility/purpose, matrix bridging ). Feature-gated `native-formats`. The GL half ( `usd_scene_load` ) is **not covered** — needs a browser |
