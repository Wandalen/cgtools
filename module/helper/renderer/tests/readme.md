...
| usd_scene_test.rs | Tests the `loaders::usd` N3 core — in-memory `ar::Resolver` stage open, fan triangulation, primvar corner resolution ( vertex / faceVarying / indexed ), `Xform` world-matrix composition, malformed-topology error paths. Feature-gated `native-formats` |
