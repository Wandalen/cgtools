...
//! `renderer::webgl::animation::loaders::gltf::{ channel_decode, vec3_sequence }` -- with zero
//! `gl`/`GL`/`WebGl` calls anywhere in their bodies. Mirrors `gltf_light_parsing_test.rs`'s
//! inline-JSON-fixture-via-`Gltf::from_slice` pattern ( task 118's own precedent ) for a second
//! pure sub-surface of this crate's glTF loading, this time the animation loader rather than the
//! light-extension loader. `quat_sequence`/`weights_sequence` are the same shape and are
//! deliberately out of scope here ( same-shape follow-up work ).

use renderer::webgl::animation::loaders::gltf::{ channel_decode, vec3_sequence };
