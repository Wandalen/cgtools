...
| `thin_film_weight`, `thin_film_thickness`, `thin_film_ior` | [KHR_materials_iridescence] | done (`iridescence_*` + `iridescenceTexture` slot) | `USE_OPENPBR_IRIDESCENCE` (analytic spectral film Fresnel, weight texture-modulated); degenerate — colorless `F_s→1` — on mirror-metal substrates |
