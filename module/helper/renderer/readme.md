...
| `fuzz_weight`, `fuzz_color`, `fuzz_roughness` | [KHR_materials_sheen] | done (`sheen_*` + `sheenColorTexture`/`sheenRoughnessTexture` slots, sheen ≈ fuzz microfiber lobe) | `USE_OPENPBR` (Charlie/Ashikhmin), texture-modulated, substrate albedo-scaled via `USE_SHEEN_ALBEDO` |
