//! Build-time WGSL→GLSL ES 300 translation for the `src/webgpu` layer's
//! WebGL override pair. That layer compiles under `feature = "webgpu"`
//! ( wasm32 ) or `feature = "native"` ( off-browser, per its `#[cfg(any(...))]`
//! gate in `src/lib.rs` ) — either one needs the generated files, so this
//! script mirrors that same `any(...)` gate rather than `webgpu` alone.
//! `renderer.rs` consumes canonical WGSL directly for the WebGPU/native/
//! Vulkan backends; WebGL needs hand-supplied GLSL
...
fn main()
{
  ...
  #[ cfg( any( feature = "webgpu", feature = "native" ) ) ]
