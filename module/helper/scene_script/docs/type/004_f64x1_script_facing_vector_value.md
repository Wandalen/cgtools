...
| File | Relationship |
|------|--------------|
| `src/vector_binding.rs` | `f64x1_register` — constructor, `.x` getter, `+`/`-` (binary)/`*` operators, unary `-` negation, `dot`/`mag`/`mag2`/`normalize`/`distance`/`distance_squared`/`min`/`max` (all native `f64`, no boundary cast), `to_string` |
