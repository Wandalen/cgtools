  ...
  impl Line
  {
    ...
    pub fn mesh_update( &mut self, gl : &gl::WebGl2RenderingContext ) -> Result< (), gl::WebglError >
    {
      ...
      if self.join_changed
      {
        ...
        match self.join 
        {
          Join::Round( _) => ( JOIN_ROUND_VERTEX_SHADER, gl::TRIANGLE_FAN ),
          Join::Miter => ( JOIN_MITER_VERTEX_SHADER,gl::TRIANGLES ),
          Join::Bevel => ( JOIN_BEVEL_VERTEX_SHADER, gl::TRIANGLES )
        };
      ...
      if self.cap_changed
      {
        ...
        let ( cap_geometry_list, _cap_indices, _cap_uvs, cap_geometry_count ) = self.cap.geometry();
