mod private
{
  ...
  pub const BODY_VERTEX_SHADER : &'static str = include_str!( "./uv/shaders/body.vert" );
   /// The vertex shader for the terminal ends of a line segment.
  pub const BODY_TERMINAL_VERTEX_SHADER : &'static str = include_str!( "./uv/shaders/body_terminal.vert" );

  /// The vertex shader for rendering a round join between line segments.
  pub const JOIN_ROUND_VERTEX_SHADER : &'static str = include_str!( "./uv/shaders/join_round.vert" );
  /// The vertex shader for rendering a mitered (sharp) join between line segments.
  pub const JOIN_MITER_VERTEX_SHADER : &'static str = include_str!( "./uv/shaders/join_miter.vert" );
  /// The vertex shader for rendering a beveled (flat) join between line segments.
  pub const JOIN_BEVEL_VERTEX_SHADER : &'static str = include_str!( "./uv/shaders/join_bevel.vert" );

  /// The vertex shader for rendering a round cap at the end of a line.
  pub const CAP_ROUND_VERTEX_SHADER : &'static str = include_str!( "./uv/shaders/cap_round.vert" );
  /// The vertex shader for rendering a square cap at the end of a line.
  pub const CAP_SQUARE_VERTEX_SHADER : &'static str = include_str!( "./uv/shaders/cap_square.vert" );
   /// The vertex shader for rendering a butt cap, which is a flat termination without any extension.
  pub const CAP_BUTT_VERTEX_SHADER : &'static str = include_str!( "./uv/shaders/empty.vert" );
}
...
crate::mod_interface!
{
  ...
  #[ cfg( all( feature = "uv", not( feature = "solid" ), not( feature = "transparent" ) ) ) ]
  exposed use
  {
    Join,
    Cap,
