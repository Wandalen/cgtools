...
void main() 
{
  ...
  vec4 clipWidthA = u_projection_matrix * ( viewA + vec4( 1.0, 0.0, 0.0, 0.0 ) * u_width );
  vec4 clipWidthB = u_projection_matrix * ( viewB + vec4( 1.0, 0.0, 0.0, 0.0 ) * u_width );

  vec2 screenA = ( 0.5 * clipA.xy / clipA.w + 0.5 ) * u_resolution;
  vec2 screenB = ( 0.5 * clipB.xy / clipB.w + 0.5 ) * u_resolution;

  vec2 screenWidthA = ( 0.5 * clipWidthA.xy / clipWidthA.w + 0.5 ) * u_resolution;
  vec2 screenWidthB = ( 0.5 * clipWidthB.xy / clipWidthB.w + 0.5 ) * u_resolution;

  float widthA = length( screenWidthA - screenA );
  float widthB = length( screenWidthB - screenB );
 // float width = mix( widthA, widthB, position.z );

  vec2 xBasis = normalize( screenB - screenA );
  vec2 yBasis = vec2( -xBasis.y, xBasis.x );

  vec2 offsetA = screenA + ( xBasis * position.x + yBasis * position.y ) * widthA;
  vec2 offsetB = screenB + ( xBasis * position.x + yBasis * position.y ) * widthB;
