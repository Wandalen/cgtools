...
- **In Scope**: `Device::new_webgpu`/`new_webgl`/`new_native`/`new_vulkan`, the two windowed constructors `new_native_windowed`/`new_vulkan_windowed`, the unified `Device::new` overloads, `Device::backend_name()`, `Surface`, `Device::depth_range()`.
...
Each non-browser backend has a second, windowed constructor returning the same triple against a real presentable surface instead of an offscreen image: `Device::new_native_windowed(window, size)` (see `docs/adr/005_windowed_native_presentation.md`) and `Device::new_vulkan_windowed(window, size)` (see `docs/adr/006_vulkan_windowed_presentation.md`). The browser backends need no counterpart — a canvas is already the presentable surface, so `new_webgpu`/`new_webgl` are windowed by construction. Neither windowed constructor takes a windowing type: `new_native_windowed` takes `impl Into<wgpu::SurfaceTarget<'static>>` and `new_vulkan_windowed` takes `&(impl HasDisplayHandle + HasWindowHandle)`, so `winit`, `sdl2` and `glfw` are all equally usable and no crate under `module/` depends on any of them. What changes downstream is only `Surface`: its `VulkanWindow`/`NativeWindow` variants make `current_view` acquire a swapchain image, `present` show it, and `resize` rebuild the chain, where the offscreen variants make `current_view` hand back the same image every frame, `present` do nothing, and `pixels_read` work (it returns `Unsupported` on a windowed surface — see `feature/006`).
