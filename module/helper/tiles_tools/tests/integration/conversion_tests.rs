...
fn test_complex_conversion_chain() {
  ...
  let square1: SquareCoord<SquareFour> = original.approximate_convert();
  let iso: IsoCoord<Diamond> = square1.convert();
  let square2: SquareCoord<SquareFour> = iso.convert();
  let hex: HexCoord<Axial, Pointy> = square2.approximate_convert();
  
  // Verify intermediate conversions are consistent
  assert_eq!(square1, square2);
  
  // Verify the final result makes sense
  assert!(hex.q <= original.x);  // Some relationship should exist
}
...
fn test_mixed_coordinate_system_operations() {
  ...
  let square_from_tri: SquareCoord<SquareFour> = tri.approximate_convert();
  ...
  assert!(square_from_tri.x.abs() <= 10);
  assert!(square_from_tri.y.abs() <= 10);
}