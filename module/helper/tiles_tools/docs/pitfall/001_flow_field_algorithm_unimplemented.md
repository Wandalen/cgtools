...
| `IntegrationField::get_cost` | `0` always | `// Simplified stub implementation - would access Grid2D` (`src/flowfield.rs:104-105`) |
| `IntegrationField::set_cost` | no-op (empty body) | `// Simplified stub implementation - would modify Grid2D` (`src/flowfield.rs:114`) |
...
| `FlowField::get_flow_direction` | `None` always | `// Simplified stub implementation - would access Grid2D` (`src/flowfield.rs:171-172`) |
| `FlowField::calculate_integration_field` (private, called by `calculate_flow`) | nothing — **empty function body**, only comments describing what a real Dijkstra pass "would" do | `src/flowfield.rs:193-203` |
| `FlowField::generate_flow_directions` (private, called by `calculate_flow`) | nothing — **empty function body**, real logic exists only as a commented-out code block | `src/flowfield.rs:213-229+` |

`calculate_flow` (the public entry point) calls exactly these two empty private functions and nothing else (`src/flowfield.rs:158-163`) — so calling it does not raise an error, but leaves the `FlowField` in the same all-default state `new()` produced. Every subsequent `get_flow_direction` call returns `None` for every coordinate, indistinguishable from a coordinate that was never part of the grid at all.
...
No test currently exercises `FlowField::calculate_flow` end-to-end and asserts a non-default `get_flow_direction` result — such a test would fail today, which is itself confirmation of the stub status.
