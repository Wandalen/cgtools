...
| `examples/minwebgpu/sun_grid_lines/scene.rhai` | The document (d2 known use) — pure literal content, no engine calls |
| `examples/minwebgpu/sun_grid_lines/src/scene.rs` | The schema + loader — no separate compiler step; `rhai::serde::from_dynamic` deserializes straight into the structs `main.rs` reads |
