...
| `tilemap_renderer` | d2 | POD `RenderCommand` stream + assets | `Backend` trait — `adapter-none`/SVG/terminal need no GPU at all (`adapter-terminal` renders onto a coarse ANSI-truecolor character-cell grid); the WebGL2 adapter uses `minwebgl` directly; `adapter-webgpu` / `adapter-native` target `gpu_hal` ( [ADR-003](../adr/003_d2_stack_hal_adoption.md) ) |
...
| `module/helper/renderer/tests/shader_validation_tests.rs` | Canonical-path WGSL shader validation (`naga`-parsed, native-only) for `src/webgpu/shaders/main.wgsl` and `tonemap.wgsl` — legacy GLSL twins (`*.vert`/`*.frag`) confirmed outside naga 30.0.0's `front::glsl` reach: that frontend targets desktop/Vulkan GLSL 440+ only (`Profile::Core`, no ES variant), empirically fails to parse all 5 real legacy shaders (unsupported `#version 300 es`, ES-only builtins like `gl_VertexID`, and missing `layout(binding=N)` decorations these OpenGL-ES-idiom shaders never declare) |
| `module/helper/renderer/tests/legacy_glsl_shader_compile_test.rs` | Closes the gap the row above leaves open: compiles all 28 shipped legacy GLSL ES 3.00 `.vert`/`.frag` files through a real headless WebGL2 context's `compileShader`/`COMPILE_STATUS`, via the crate's own production compile path (`gl::ShaderSource`) — the actual GLSL ES 3.00 compiler these sources target, not an offline stand-in; no external tool (e.g. `glslangValidator`) needed |
| `module/helper/renderer/tests/skeleton_tests.rs` (also has a `wasm-bindgen-test` half alongside its native `pure_tests` — needs a live `WebGl2RenderingContext`), `tests/animation_graph_tests.rs`, `tests/gltf_animation_loader_test.rs`, `tests/mirror_tests.rs`, `tests/scaler_tests.rs`, `tests/color_grading_tests.rs`, `tests/webgl_frame_orchestration_test.rs` | Legacy-path native coverage of scene-graph, skeletal, animation-loading, post-processing, and frame-orchestration vocabulary — representative, not exhaustive |
