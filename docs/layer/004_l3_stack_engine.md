...
| `module/helper/renderer/tests/shader_validation_tests.rs` | Canonical-path WGSL shader validation (`naga`-parsed, native-only) for `src/webgpu/shaders/main.wgsl` and `tonemap.wgsl` — legacy GLSL twins (`*.vert.glsl`/`*.frag.glsl`) are outside naga's reach, need the Khronos reference validator instead |
