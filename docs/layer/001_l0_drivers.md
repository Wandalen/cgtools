...
| `module/min/minvulkan/` | Native Vulkan driver via `ash`, `wgpu`-free — real Instance/Device/Queue, tested against a live ICD; resource construction (buffers, images, pipelines) not yet implemented at this layer (already exists in `gpu_hal`'s `vulkan` backend); swapchain/present-queue-backed presentation is offscreen-only and unimplemented anywhere in the stack |
