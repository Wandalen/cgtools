...
| 004 | [Native Vulkan HAL Backend via a Dedicated `minvulkan` Driver](004_native_vulkan_hal_backend.md) | `gpu_hal` gains a fourth, `wgpu`-free backend (`minvulkan` via `ash`) so `examples/orrery/flexible` can offer Vulkan without linking `wgpu` | ✅ Accepted |
| 005 | [Windowed Native Presentation via Handle Traits, Not a Windowing Driver](005_windowed_native_presentation.md) | `minwgpu` gains a windowed surface and frame lifecycle, `gpu_hal` a `NativeWindow` surface — a window enters as handle traits, so no crate under `module/` depends on a windowing library | ✅ Accepted |
| 006 | [Vulkan Windowed Presentation via a Real `VK_KHR_swapchain`](006_vulkan_windowed_presentation.md) | `minvulkan` gains surface and swapchain, `gpu_hal` a `VulkanWindow` surface — closing the leg ADR-005 deferred, so a windowed process can link no `wgpu` at all | ✅ Accepted |
